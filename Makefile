...
.PHONY: compile build run deps updatedeps testdeps golint vet goimports goimports-check test test-coverprofile bench coverage clean tidy tidy-check
...
build:
	...
	GOOS=darwin $(GOBUILD) \
		...
		-ldflags="-w \
				  -s \
                  -X github.com/muffix/mvg-info/internal/httpapi.revision=${GITHUB_SHA} \
                  -X github.com/muffix/mvg-info/internal/httpapi.pipelineID=${GITHUB_RUN_ID}_${GITHUB_RUN_NUMBER} \
...
run:
	$(GORUN) \
		...
		-ldflags="-w \
				  -s \
				  -X github.com/muffix/mvg-info/internal/httpapi.revision=dev \
...
tidy-check: tidy
	@if [ -n "$$(git diff-index --ignore-submodules --name-only HEAD | grep -E '^go.(mod|sum)$$')" ]; then \
